...
steps <- list(
  list(label = "Step 1  Grid  7 (model x think) x 3 temps x 22 papers",
       df = tuning_df, multiplier = n_model_configs * n_temps),
  list(label = "Step 2  Prompts  7 models x 3 formats x 22 papers",
       df = tuning_df, multiplier = n_model_configs * n_prompt_fmts),
  list(label = "Step 3  Full GT  7 models x GT papers",
       df = gt_df,     multiplier = n_model_configs),
  list(label = "Step 4  Stability  7 models x 5 runs x 22 papers",
       df = tuning_df, multiplier = n_model_configs * n_stability_runs)
)
...
# ── Per-paper breakdown (tuning subset) ───────────────────────────────────────
cat(sprintf("\n%s\n  Per-paper profile (tuning subset)\n%s\n", divider, divider))
cat(sprintf("  %-30s  %6s  %7s  %5s  %5s  %5s\n",
            "paper_id", "files", "calls", "p1", "agg", "gran"))
for (i in seq_len(nrow(tuning_df))) {
  r <- tuning_df[i, ]
